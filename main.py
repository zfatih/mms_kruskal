...
class MainView(tk.Frame):
    def __init__(self, root):
        ...
        label = tk.Label(self, text="Broj čvorova(5-60):", font=20)
        label.place(x=10, y=20)

        vcmd = (self.register(self.onValidate), '%d', '%i', '%P', '%s', '%S', '%v', '%V', '%W')
        self.entry = tk.Entry(self, validate="key", validatecommand=vcmd, font=20)
        self.entry.place(x=160, y=21)

        self.b1 = tk.Button(self, text="START", command=self.runAnimation, font=20, height=4, width=15)
        self.b1.place(x=110, y=90)

        self.errorPoruka = tk.StringVar()

        self.text = tk.Label(self, textvariable=self.errorPoruka, foreground='red', font=20)
